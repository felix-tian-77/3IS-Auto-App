...
### Requirement: 搜索与筛选
系统 SHALL 支持按手机号或申请编号搜索，以及按状态和业务类型筛选。

#### Scenario: 按手机号搜索
- **WHEN** 用户在搜索框输入 "138" 并点击查询
- **THEN** 系统调用 API 传入 phone 参数，表格刷新为匹配结果

#### Scenario: 按状态筛选
- **WHEN** 用户选择状态筛选为"已完成"并点击查询
- **THEN** 系统调用 API 传入 status 参数，表格仅显示已完成状态的申请
