...
**文档状态机：** `Draft` → `Review` → `Approved` → `Frozen`。任何重大修订必须先回退到 `Draft`，再走一轮同行评审。当前状态为 `Draft`，正在应用 2026-06-03 架构调整方案，等待业务方、合规方、运维方三方签字后进入 `Review` / `Approved`。
...
销售人员提交资料(指定业务类型) → [事务接入] → 智能路由(按权重) → Schema校验
    ...
    → 调用OSS签名URL API获取下载链接
    → Socket通知Device下载（Device直连OSS）
...
1. 调用 `POST /api/v1/transactions/{id}/oss-urls` 获取 OSS 签名 URL 列表
2. 启动 Airtest runtime：`connect_device("android:///{adb_serial}")`
3. 通过 Socket :8765 将 URL 列表下发给 Device（Device 直连 OSS 下载，不经 Worker 中转）
...
#### FR-MOB-002 文件同步（V1.2 简化）

| 字段 | 内容 |
|------|------|
| 描述 | 根据 Transaction ID 从 **OSS 签名 URL** 下载输入影像到 Device 本地沙箱（V1.1 → V1.2 不变，**Device 仍直连 OSS**） |
| 输入 | Transaction ID、OSS 签名 URL 列表（含 MD5） |
| 输出 | 本地沙箱中的文件（默认 `/sdcard/sandbox/{txn_id}/`） |
| 规则 | 直连 OSS 签名 URL 下载（**V1.2 仍由 Device 直连，不经 Worker**）；支持断点续传；下载完成后做 MD5 校验；存储路径可配置；签名 URL 过期（默认 5min）自动调用续签接口 |
...
#### Socket 通道（Android ↔ Worker，V1.2 精简：移除 EXECUTE_STEP / STEP_RESULT）

| 方向 | 用途 |
|------|------|
| Worker → Device | **V1.2 仅保留**：下发下载指令（DOWNLOAD_FILES） |
| Device → Worker | **V1.2 仅保留**：上报下载完成（DOWNLOAD_COMPLETE）、OSS URL 过期（OSS_URL_EXPIRED） |

**消息格式：**

```json
// Worker → Device (LAN Socket :8765) — V1.2 唯一保留的指令
{
  "cmd": "DOWNLOAD_FILES",
  "params": {
    "transaction_id": "TXN-20260603-00001",
...
### 7.1 逻辑架构图（V1.2 更新：Worker 端 Airtest runtime）
...
### 7.5 硬件建议（V1.2 上调 Worker 规格、下调 Device 规格）
...
| 脚本存储规范 | 流程脚本包（.py/.zip）存储于 OSS，目录结构：`/flows/{flow_id}/{version}/script.zip`；本地缓存于 **Worker（V1.2 强调）**：`~/.3is-auto/flows/{flow_id}/{version}/` |
| **沙箱路径（V1.2 新增）** | **Worker 本地：`~/.3is-auto/sandbox/{txn_id}/screenshots/`（Airtest 截图）；Device：`/sdcard/sandbox/{txn_id}/`（输入影像）** |
...
| **成本上限（V1.1 新增，V1.2 调整）** | **MVP 阶段月成本 ≤ ¥12000**（**V1.2 上调**，原 ¥8000）：含 ECS 2x、RDS、Redis 4GB、OSS 按量、SLB、KMS、**Worker 2-3 台 × ¥600/月 = ¥1200-1800**（**V1.2 新增**） |
...
| **架构变更发布（V1.2 新增）** | **V1.1 → V1.2 升级采用双轨并行（V1.1 架构保留为 Plan B），3 个月观察期后切流到 V1.2** |
...
| 前一版本 PRD | `docs/superpowers/specs/2026-06-02-prd-design.md`（V1.1） | V1.2 的修订基础 |
